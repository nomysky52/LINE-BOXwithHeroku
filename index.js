...
bot.on('message', function(event) {
    ...
    switch (event.message.type) {
        case 'text':
            ...
            switch (event.message.text) {                
                case '素食說明':
                    event.reply('[包裝食品的素食標示-2014.11.05修編]素食產品' + '\n' 
+ '標示將分為「全素或純素」、「蛋素」、「奶素」、「奶蛋素」及「植物五辛素」五類' + '\n' + '\n'
+ '全素或純素：不含奶蛋、也不含五辛（蔥、蒜、韭、薤菜及興蕖）的純植物性食品。' + '\n'
+ '蛋素：全素或純素及蛋製品。' + '\n'
+ '奶素：全素或純素及奶製品。' + '\n'
+ '奶蛋素：全素或純素及奶蛋製品。' + '\n'
+ '植物五辛素：指食用植物性食物，但可含五辛（蔥、蒜、韭、薤菜及興蕖）或奶蛋。' + '\n');
                    event.reply('二十二碳六烯酸（DHA）和二十碳五烯酸（EPA）是從魚油萃取而來，而魚油萃取過程中，魚已被犧牲。屬於葷食。');
                    event.reply('食用天然色素胭脂紅是從胭脂蟲提煉而來，而食用天然色素胭脂紅提煉過程中，胭脂蟲已被犧牲。屬於葷食。');
                    event.reply('大部分茹素者「戒酒」，因此食品中如含有酒作為原料，不宜標示為素食。');
                    event.reply('素食者有分為' + '\n' 
